...
interface Window {
  electronAPI: {
    ...
    terminalWrite: (sid: number, data: string) => void;
    terminalResize: (sid: number, rows: number, cols: number) => void;
    terminalKill: (id: number) => void;
    onTerminalData: (callback: (sid: number, data: string) => void) => void;
  };
}
