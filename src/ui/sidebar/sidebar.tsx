...
import { MoreHorizontal, FolderOpen, ChevronRight } from 'lucide-react';
...
import { RootFolder } from './components/root_folder';
import { InputField } from './components/input_field';
import { useSidebarVirtualization, VirtualRow } from './hooks/use_sidebar_virtualization';


// --- Redundant FileRow removed and moved to ./components/file_row.tsx ---
...
export const Sidebar: React.FC = () => {
  ...
  const { 
    visibleItems, totalHeight, startIndex, flattenedVisibleNodes 
  } = useSidebarVirtualization(
    fileTree, expandedFolders, newInput, scrollTop, containerHeight
  );
  ...
  }, [flattenedVisibleNodes, expandedFolders, setSelectedPaths, setSelectedPath, toggleFolder, updateNode, openFile]);
  ...
  // Virtualization math removed - handled by useSidebarVirtualization hook
        ...
        >
          {fileTree ? (
            <div className="pb-4">
              <RootFolder 
                fileTree={fileTree}
                isExpanded={expandedFolders.includes(fileTree.path)}
                isSelected={selectedPath === fileTree.path}
                onToggle={toggleFolder}
                onStartFile={() => startCreation('file')}
                onStartFolder={() => startCreation('folder')}
